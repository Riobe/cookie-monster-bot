...
exports.eatCookie = function (message) {
  ...
  let error = user.eatCookie()

  if (error) {
    winston.warn('User got error while eating a cookie.', { error: error });
  } 

  message.reply(error || `COOKIE! NOM NOM NOM NOM!  YOU HAVE ${user.cookies.toLocaleString()} ${getCookieGrammar(user).toUpperCase()} LEFT!`);
};
